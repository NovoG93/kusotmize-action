...
func ensureKustomize(version string, expectedSHA256 string) error {
	...
	url := fmt.Sprintf("https://github.com/kubernetes-sigs/kustomize/releases/download/kustomize%%2F%s/kustomize_%s_linux_amd64.tar.gz", version, version)
	...
	cmd := exec.Command("tar", "-xzf", tmpPath, "-C", "/usr/local/bin")
	if output, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("extract failed: %w: %s", err, strings.TrimSpace(string(output)))
	}

	bin := filepath.Join("/usr/local/bin", "kustomize")
